...
uint64 map_shared_pages(struct proc* src_proc, struct proc* dst_proc, uint64 src_va, uint64 size) {
  ...
  uint64 dst_end = dst_start + (src_end - src_start); // dst_start + (src_va - src_start) + size;
  uint64 pa;
  pte_t *src_pte;
  int flags;

  for (uint64 addr = src_start; addr < src_end; addr += PGSIZE) {
    src_pte = walk(src_proc->pagetable, addr, 0);
    if (src_pte == 0) {
      return -1; // PTE does not exist
    }
    if ((*src_pte & PTE_V) == 0 || (*src_pte & PTE_U) == 0) {
      return -1; // PTE not valid or not shared
    }
    // get the PA and flags of the page
    pa = PTE2PA(*src_pte);
    flags = PTE_FLAGS(*src_pte);
    // map the page to the destination process
    if(mappages(dst_proc->pagetable, dst_start, PGSIZE, (uint64)pa, flags|PTE_S) != 0){
      // unmapping the pages that were already mapped
      uvmunmap(dst_proc->pagetable, dst_proc->sz, (addr - src_start) / PGSIZE, 1);
      return -1;
    }
    dst_start += PGSIZE;
  }
